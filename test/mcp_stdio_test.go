...
func TestMCPClientStdioInitializeAndCall(t *testing.T) {
	...
	if len(tools) == 0 || tools[0].Name != "dir2mcp.ask" {
		t.Fatalf("unexpected tools: %+v", tools)
	}

	res, err := client.CallTool(context.Background(), "dir2mcp.ask", map[string]any{"question": "hello", "k": 8})
	if err != nil {
...
func TestHelperProcessMCPStdio(t *testing.T) {
	...
	for {
		...
		case "notifications/initialized":
			continue
		case "tools/list":
			helperWriteJSON(map[string]any{"jsonrpc": "2.0", "id": id, "result": map[string]any{"tools": []map[string]any{{"name": "dir2mcp.ask", "description": "ask"}}}})
		case "tools/call":
